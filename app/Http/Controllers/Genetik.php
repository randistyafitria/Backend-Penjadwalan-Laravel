...
//print_r($individu_baru[$i]);die;
    ...
    public function Mutasi($fitness)
    {
        ...
        if ($this->found_while_checked == false and
...
// ";
    ...
    //     ->where('matkuls.semester', '%%', 2, '=', 0)
    //     ->where('pengampus.tahun_akademik', '=', '%s')  
    //     ->get();   /////
    // }
    ...
    // {
    //     $jadkuls = Jadkul::insert('insert into jadkuls (kode_pengampu, kode_jam, kode_hari, kode_ruang) values (%s, %s, %s, %s)');  // %s ?
    // }
    ...
    //         'hari.nama as hari',
    //         //'#CONCAT_WS('-', CONCAT('(', jams.id), CONCAT( (SELECT id FROM jams WHERE id = (SELECT jam.id FROM jams jam WHERE MID(jam.range_jam,1,5) = MID(jams.range_jam,1,5)) + (matkuls.sks - 1)),')')) AS sesi',
    //         '#CONCAT_WS('-', MID(jams.range_jam,1,5), (SELECT MID(range_jam,7,5) FROM jams WHERE id = (SELECT jam.id FROM jams jam WHERE MID(jam.range_jam,1,5) = MID(jams.range_jam,1,5)) + (matkuls.sks - 1))) AS jam_kuliah',
    //         //'CONCAT('(',jams.kode, '-', jams.id + matkuls.sks - 1,')') AS sesi',
    //         'CONCAT_WS('-', MID(jams.range_jam,1,5), (SELECT MID(range_jam,7,5) FROM jams WHERE id = jams.id + matkuls.sks - 1)) AS jam_kuliah',
